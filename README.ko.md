...
v38은 Android와 같은 GitHub Latest Release API와 `useAPIAssets` 설정을 사용한다. 게임 시작 후
백그라운드에서 최신 Release의 ZIP을 확인하며 새 버전은 앱의 `Library/Application Support`에
버전별로 내려받는다. ZIP 내부 버전과 필수 iOS 인덱스를 모두 검증한 뒤 완료 표식을 기록하고,
다음 실행부터 다운로드 데이터를 IPA 내장 데이터보다 우선해서 읽는다. 실패하거나 중단되면
내장 데이터로 계속 실행한다. 설정에는 현재 번역 데이터 버전, 새 번역 데이터 버전, 업데이트
상태를 표시한다.

v40은 Android의 `targetFrameRate` 동작을 정적 훅으로 이식한다. iOS 설정 앱의 `아이프라`
항목에서 최대 FPS를 숫자로 입력할 수 있다. Solis 그래픽 설정과 Unity의 최종 프레임 제한을
모두 가로챈다. 패키징 시
원본 IPA에서 꺼져 있는 `CADisableMinimumFrameDuration`과
`CADisableMinimumFrameDurationOnPhone`을 활성화해 ProMotion 기기에서 120Hz 요청을 허용한다.

v41은 Android의 `라이브 결과로 바로 이동`을 이식한다. 개발자 기능에서 고급 기능 코드가
정확히 `619`이고 해당 토글도 켜진 경우에만 `LiveUtility.LoadLiveScene`을 가로채
`LiveUtility.LoadLiveResult`로 바로 전환한다. 코드가 다르거나 토글이 꺼져 있으면 원본 경로를
그대로 실행한다.

v42는 원본 IPA에서 꺼져 있던 iPad·iPhone용 ProMotion 허용 키를 켠다. 기존 FPS 훅은 그대로
사용하며, 설정한 FPS가 기기에서 지원되는 경우 60Hz보다 높은 화면 갱신을 요청할 수 있다.

v44는 iOS에서 안정적으로 적용되지 않던 화면 방향 고정 설정과 관련 훅을 제거한다. 숫자로
지정하는 FPS 제한 변경과 ProMotion 허용 처리는 유지한다. 2026-09-20 사용자가 120 FPS와
나머지 패치 기능의 정상 동작을 실기에서 확인했다.
...
Android는 추가 디렉터리를 사용하지 않으며 iOS는 대용량 JSON을 매 실행마다 파싱하지 않는다.
...
`{user}`를 표시용 사용자명으로 바꾸고, 바꾼 이름을 기준으로 조사를 결정한다.

v31은 설정 → 앱 → 아이프라 → 표시용 사용자명에서 `displayUserName`을 읽는다.
공란이면 게임의 이름을 유지하며, 값을 넣으면 다음 실행부터 ADV 이름과 메시지·알림 인자를 교체한다.
게임 계정의 실제 이름을 변경하지 않는다. 최종 텍스트의 `{user}` 치환 후 새 이름으로 조사를 선택한다.
알림은 Android와 동일하게 원본 함수 반환 후 조사만 처리하며 generic/문장부호 치환은 추가하지 않는다.
로그의 `USERNAME ARMED: ADV/message/notification; custom=ON`으로 설정 적용을 확인한다.
이름 자체는 로그에 출력하지 않는다. 설정 입력은 UTF-8 4095바이트 이내이며 초과하면 게임 이름을 유지한다.

일반 ADV의 getter는 제네릭 부모 `Solis.OutGame.ADVEnginePresenterBase<TView>`의 공유 구현이다.
6.0.2 메타데이터 methodDef 54086 / token 0x0600d347, methodSpec 21875,
generic function index 20631에서 `0x4847624`를 확인했다. 빌드 시 이 연결과 함수 바이트를 검증한다.
...
chcp 65001 > $null
./build-hook.ps1
python tools/package_probe.py --ipa dump/ios/game.qualiarts.idolypride-6.0.2-Decrypted.ipa --dylib build/hook-v2/HoshimiLocalify.dylib --hook-plan build/hook-v2/hook-plan.json --font-file PretendardJP-SemiBold.otf --local-data-root hoshimi-local --include-adv --include-master --include-images --include-phone-subtitles --patch-revision 44 --output build/IdolyPride-6.0.2-HoshimiHook-v44.ipa
```
...
서브레포 자체의 GitHub Actions는 `version.txt`, `local-files`, `ios-data`를 하나의 Release ZIP으로
배포한다. iOS는 Android와 같은 Latest Release API에서 이 ZIP을 받고 앱 데이터 영역의 검증된
업데이트본을 IPA 내장본보다 우선해서 읽는다.
